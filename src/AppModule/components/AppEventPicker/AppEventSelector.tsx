...
export interface AppEventSelectorProps {
    show: boolean;
    handleClose: (show: boolean) => void;
    handleSelected: (items: Conference[]) => void;
    items: Conference[];
    selectedItems: Conference[];
    totalItem?: number;
...
export const AppEventSelector: FC<AppEventSelectorProps> = ({
    show,
    handleClose,
    items,
    handleSelected,
    ...
    const addActiveId = (id: number) => {
        const item = items.find((i: Conference) => i.id === id);

        handleSelected([
            ...(selectedItems as Conference[]),
            item as Conference,
        ]);
    };

    const getActive = (id: number): boolean => {
        const item = selectedItems.find((i: Conference) => i.id === Number(id));
                        ...
                        <div className="select-box--header">
                            <div className="row m-0 p-0">
                                <div className="title col-6">
                                    <h4 className="mb-0">
                                        {t("exhibitor.form:selected.events")}
                                    </h4>
                                </div>
                            </div>
