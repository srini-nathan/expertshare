...
export enum AppAvatarVariant {
    ROUNDED = "rounded",
    CIRCLE = "circle",
    THUMBNAIL = "thumbnail",
}

export interface AppAvatarProps extends ImageProps {
    className?: string;
    src?: string;
...
const Avatar = styled(Image)<AppAvatarProps>(({ fluid, size }) => {
    if (fluid) {
        return {};
    }
    // in rem, 1rem = 16px, 2.75rem = 44px
    let baseSize = 2.75;
...
export const AppAvatar: FunctionComponent<AppAvatarProps> = (props) => {
    ...
    return (
        <Avatar
            ...
            rounded={!variant || variant === AppAvatarVariant.ROUNDED}
            roundedCircle={variant === AppAvatarVariant.CIRCLE}
            thumbnail={variant === AppAvatarVariant.THUMBNAIL}
            {...props}
        />
    );
};
